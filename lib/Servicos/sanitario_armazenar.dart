...
class AvaliadorStorage {
  final FirebaseUser user;

  AvaliadorStorage.forUser({
    @required this.user,
  }) : assert(user != null);

  static AvaliadorItem fromDocument(DocumentSnapshot document) =>
      _fromMap(document.data);

  static AvaliadorItem _fromMap(Map<String, dynamic> data) =>
      new AvaliadorItem.fromMap(data);

  Map<String, dynamic> _toMap(AvaliadorItem item,
  ...
  Future<TodoItem> create(String title) async {
    final TransactionHandler createTransaction = (Transaction tx) async {
      ...
      final TodoItem newItem =
          new TodoItem(id: newDoc.documentID, title: title);
  ...
  Future<bool> update(AvaliadorItem item) async {
    final TransactionHandler updateTransaction = (Transaction tx) async {
      final DocumentSnapshot doc =
          await tx.get(todoCollection.document(item.id));
      // Permission check
